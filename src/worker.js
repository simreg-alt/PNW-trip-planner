...
const TOOLS = [
  ...
  {
    name: "update_booking",
    description: "Update or add a lodging/campground booking in the trip database.",
    input_schema: {
      type: "object",
      properties: {
...
async function chat(env, body) {
  ...
  for (let round = 0; round < 5; round++) {
    ...
    const customTools = toolUses.filter((t) => t.name === "update_booking" || t.name === "add_open_item");
...
function buildSystemPrompt(state) {
  const bookings = state.bookings.map((b) =>
    `- ${b.date_label}: ${b.name}${b.site ? " (site " + b.site + ")" : ""}${b.confirmation ? " conf " + b.confirmation : ""} [${b.status}, ${b.hookups || "?"} hookups]`
  ).join("\n");
  const items = state.openItems.map((i) => `- ${i.text}`).join("\n");
...
You can: answer questions about the itinerary, use web_search for live info (weather, road/tide conditions, hours, prices), and edit the plan via the update_booking and add_open_item tools. When you change a booking, state clearly what you changed. Never give drive times from memory — search or tell the user it needs live verification. Keep answers concise and practical. Respect the family's constraints (kosher, Shabbat timing, modesty, paved roads, kid pacing) in every suggestion.`;
}
